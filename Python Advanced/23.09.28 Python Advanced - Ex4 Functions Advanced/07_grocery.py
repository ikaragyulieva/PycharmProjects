...
# If there are two or more products with the same name's length, sort them by their name in ascending order
...
def grocery_store(**kwargs):
    result = []
    recipe = dict(sorted(kwargs.items(), key=lambda kvp: (-kvp[1], -len(kvp[0]), kvp[0])))
