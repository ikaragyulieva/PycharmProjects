...
# o	You are not supposed to check the weight of the un accommodated pets (if any) when you run out of space.
#  In the end:
...
# •	On the following lines return the accommodated pet types and number of pets, ordered ascending (alphabetically)
# by pet type. Each on a new line:
# "Accommodated pets:
# {pet_type1}: {number}
# {pet_type2}: {number}
# …
...
def accommodate_new_pets(hotel_capacity, weight_limit, *args):
    ...
    accommodated_pets = {}
    pending_pets = deque(args)
    hosted_pets = 0
    while pending_pets:
        if pending_pets[0][1] <= weight_limit:
            if pending_pets[0][0] not in accommodated_pets:
                accommodated_pets[pending_pets[0][0]] = 0
            accommodated_pets[pending_pets[0][0]] += 1
            hosted_pets += 1
        pending_pets.popleft()

        if hotel_capacity == hosted_pets:
            break
    sorted_accommodated = sorted(accommodated_pets.items(), key=lambda kvp: kvp[0])
    result = ''
    if len(pending_pets) == 0:
        result += f"All pets are accommodated! Available capacity: {hotel_capacity-hosted_pets}.\n"
    else:
        result += "You did not manage to accommodate all pets!\n"
    result += "Accommodated pets:\n"
    for pet_type, number in sorted_accommodated:
